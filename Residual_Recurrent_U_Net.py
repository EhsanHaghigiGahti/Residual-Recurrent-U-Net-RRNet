...
def build_R2Net(input_shape):
    ...
    model = Model(inputs, outputs, name="R2-Net")
    return model
...
model.compile(optimizer=Adam(learning_rate = 1e-4), loss='binary_crossentropy', metrics=['accuracy',IoU,F1_Score,recall,precision,overall_accuracy])

from keras.callbacks import ModelCheckpoint, CSVLogger
filepath = "#Outputfile path#/Model_R2Net-{epoch:03d}-{val_loss:.3f}.hdf5"
checkpoint = ModelCheckpoint(filepath, monitor='val_loss', verbose=1, save_best_only=False, save_freq="epoch", mode='min')
log_csv = CSVLogger('#Outputfile path#/Model_R2Net_Log.csv', separator=',', append=False);
